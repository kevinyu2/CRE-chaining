...
# Finds pairwise for all ACRs. Uses local anchor structure (indexed by motif number instead of position)
# Make sure to clear the output folder first since we are appending to files 
# Printed output is a file for each chr chr pair, each line has genome\tloc\tgenome\tloc\t...genome\tloc##genome\tloc\t....genome\tloc\nscore\n       
def find_anchors_all_pairs_local(data_dir, out_dir, score_dict) :
    # Holds where each motif is located {MOTIF: {chr : [(genome, loc), (genome, loc)]), chr2 : [...]}, MOTIF2 ...}
    # Since this is 
    motif_loc_dict = defaultdict(lambda: defaultdict(list))
    ...
    for file_path in Path(data_dir).rglob("*"):
        if file_path.is_file() and file_path.suffix == ".tsv":
            ...
            with open(file_path, "r") as f:
                ...
                for line in f:
                    line_arr = line.rstrip().split('\t')
                    starts.add(int(line_arr[1]))
            
            motif_indexes = {v: i for i, v in enumerate(sorted(starts))}
            ...
            with open(file_path, "r") as f:
                ...
                for line in f:
                    # Each line is motif, location
                    line_arr = line.rstrip().split('\t')
                    motif_loc_dict[line_arr[0]][folder_name].append((file_name, motif_indexes[int(line_arr[1])]))
...
OUT_DIR = '/home/mwarr/Data/Anchors_min1_local'
MIN_SCORE = 1.0
...
find_anchors_all_pairs_local(DATA_DIR, OUT_DIR, score_dict)
